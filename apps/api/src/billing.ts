...
export function registerBilling(app: FastifyInstance) {
  ...
  app.post('/v1/subscriptions/:id/run_cycle', async (request, reply) => {
    ...
    try {
      const row = await prisma.$transaction(async (tx) => {
        ...
        return { kind: 'advanced' as const, subscription: updated };
      }, { isolationLevel: Prisma.TransactionIsolationLevel.Serializable });

      if (row.kind === 'missing') return reply.code(404).send({ error: { type: 'not_found', message: 'No such subscription.' } });
      if (row.kind === 'inactive') return reply.code(409).send({ error: { type: 'invalid_state', message: 'Only active subscriptions can run another billing cycle.' } });
      if (row.kind === 'canceled') {
        ...
        const full = await prisma.subscription.findUniqueOrThrow({ where: { id }, include: { invoices: { include: { paymentIntent: true }, orderBy: { createdAt: 'desc' }, take: 1 } } });
        return { result: 'canceled_at_period_end', subscription: serializeSubscription(full) };
      }
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError && (error.code === 'P2002' || error.code === 'P2034')) {
        return reply.code(409).send({ error: { type: 'conflict', message: 'This subscription cycle was already generated or is being generated by another request.' } });
      }
      throw error;
    }
  });
  ...
  app.get('/v1/invoices', async (request, reply) => {
    const auth = await merchantAuth(request);
    if (!auth) return reply.code(401).send({ error: { type: 'authentication_error', message: 'A valid test API key or merchant session is required.' } });
    const rows = await prisma.invoice.findMany({ where: { merchantId: auth.merchantId }, include: { paymentIntent: true }, orderBy: { createdAt: 'desc' }, take: 100 });
  ...
  app.post('/v1/invoices/:id/void', async (request, reply) => {
    ...
    if (invoice.status !== 'OPEN' || invoice.amountPaid > 0n) return reply.code(409).send({ error: { type: 'invalid_state', message: 'Only an unpaid open invoice can be voided.' } });
    const row = await prisma.invoice.update({ where: { id }, data: { status: 'VOID' }, include: { paymentIntent: true } });
    if (row.paymentIntentId) await prisma.paymentIntent.updateMany({ where: { id: row.paymentIntentId, status: { in: ['REQUIRES_PAYMENT_METHOD', 'REQUIRES_CONFIRMATION', 'REQUIRES_ACTION'] } }, data: { status: 'CANCELED' } });
