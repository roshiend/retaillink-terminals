...
export default function DashboardPage() {
  ...
  async function revokeKey(id: string) {
    ...
    await request(`/dashboard/api_keys/${id}`, { method: 'DELETE' }); await refreshAll();
  }
  ...
  async function deleteWebhook(id: string) {
    ...
    await request(`/v1/webhook_endpoints/${id}`, { method: 'DELETE' }); await refreshAll();
  }
  ...
  return <main className="appShell">
    <aside className="sidebar">
      ...
      <nav>
        {(['overview','payments','developers','webhooks','audit'] as Section[]).map((item) => <button key={item} className={section === item ? 'navActive' : ''} onClick={() => setSection(item)}>{item[0].toUpperCase() + item.slice(1)}</button>)}
      </nav>
    ...
    <section className="mainArea">
      ...
      {section === 'developers' && <>
        <section className="panel"><p className="eyebrow">API access</p><h2>Sandbox secret keys</h2><p className="muted">Use secret keys only from your server. New secrets are shown once and stored only as a hash.</p><form className="inlineForm" onSubmit={createApiKey}><label>Key name<input value={keyName} onChange={(e) => setKeyName(e.target.value)} /></label><button disabled={busy}>Create key</button></form><div className="tableWrap"><table><thead><tr><th>Name</th><th>Key prefix</th><th>Last used</th><th>Status</th><th></th></tr></thead><tbody>{apiKeys.map((key) => <tr key={key.id}><td><strong>{key.name}</strong><small>{new Date(key.created_at).toLocaleString()}</small></td><td><code>{key.prefix}…</code></td><td>{key.last_used_at ? new Date(key.last_used_at).toLocaleString() : 'Never'}</td><td><span className={`status ${key.revoked_at ? 'failed' : 'succeeded'}`}>{key.revoked_at ? 'revoked' : 'active'}</span></td><td>{!key.revoked_at && <button className="linkButton danger" onClick={() => void revokeKey(key.id)}>Revoke</button>}</td></tr>)}</tbody></table></div></section>
